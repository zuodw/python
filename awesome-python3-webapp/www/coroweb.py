...
# POSITIONAL_ONLY          位置参数(python历史产物，不重要)
# KEYWORD_ONLY             命名关键字参数（这种参数只会在VAR_POSITIONAL类型参数后面而且不带**前缀）
...
def has_named_kw_arg(fn):  # 判断是否有命名关键字参数
    for name, param in inspect.signature(fn).parameters.items():
        if param.kind == inspect.Parameter.KEYWORD_ONLY:
            return True
...
def add_routes(app, module_name):
    n = modele_name.rfind('.') # 从右侧检索，返回索引。若无，返回-1。
    # 导入整个模块
    if n == -1:
